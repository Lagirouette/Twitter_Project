...
import { CreateNewPost, PostImage, UpdateProfile } from "@/api/ApiCalls";
...
export default function Edit({
    ...
    const submit = async (e:SyntheticEvent) => {
        ...
        if (focus == "Twett"){
            try{
                if(token){ 
                    const post = await CreateNewPost(Input)
                    if(image){
                    ...
                    <form action="" onSubmit={submit}>
                        ...
                        <div className='flex w-full pt-2'>
                            ...
                            <div className='ms-auto flex'>
                                ...
                                <input className="hidden" type="file" id="image_uploads" name="image_uploads"/>
                                <span className='mr-2 h-full text-2xl'>|</span>
                                <button type='submit' className=' bg-sky-500 hover:bg-sky-800 rounded-full py-2 px-3 place-self-end font-bold'>{focus}</button>
                            </div>
